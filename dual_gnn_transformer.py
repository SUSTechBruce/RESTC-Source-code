...
from new_utils import trans_to_cuda
...
class DualSSlGloablModel(torch.nn.Module):
    ...
    def __init__(self, args, global_graph, num_nodes, adj_all, weight_all):
        ...
        self.beta = 0.00025 # beta: 0.005 for diginetica  0.00025 for Tmall ssl task maginitude  
...
############################################## 对比学习部分代码 ##########################################################
        ...
        gat_concat_hs_pos = gat_hidden # concat in dimension
        ...
        if is_train:
            contrast_loss = self.batch_contrastive_learning(GAT_emb, trans_emb)
    ...
    def batch_contrastive_learning(self, gat_emb, trans_emb):
        
        def cosine_sim_score(x1, x2):
            x1 = F.normalize(x1, p=2, dim=-1)
            x2 = F.normalize(x2, p=2, dim=-1)
            return torch.sum(torch.mul(x1, x2), -1)

        def negs_sampling(embeds, sample_nums):
            # batch dim
            sample_list = [embeds[torch.randperm(embeds.size()[0])] for _ in range(sample_nums)]
           
            negs = torch.stack(sample_list, dim=0) # samples, batch ,dim

            corrupted_negs = negs[:, :, torch.randperm(embeds.size()[-1])]

            negs_pool = torch.cat((negs, corrupted_negs), 1)

            sample_index = torch.LongTensor(random.sample(range(negs_pool.shape[1]),sample_nums))
            sample_index = trans_to_cuda(sample_index)
            neg_samples = torch.index_select(negs_pool, 1, sample_index)

            neg_samples = torch.reshape(neg_samples, (embeds.shape[0], sample_nums, embeds.shape[-1]))

            return neg_samples # samples, batch, dim
        
        batch_size = gat_emb.shape[0]
        assert gat_emb.shape[0] == trans_emb.shape[0]
        sample_nums = batch_size
        Temp = 0.3
        scale = math.sqrt(gat_emb.shape[-1])

        pos = gat_emb # batch dim
        pos_samples = trans_emb # batch dim
        neg_samples = negs_sampling(trans_emb, sample_nums) # batch samples dim

        # calculate positive scores
        pos_score = cosine_sim_score(pos, pos_samples)
        pos_score = torch.exp(pos_score / Temp)

        # calculate negatives scores
        neg_score = cosine_sim_score(pos.unsqueeze(1).repeat(1, sample_nums, 1), neg_samples)
        
        neg_score = torch.sum(torch.exp(pos / Temp), 1)
        con_loss = -torch.sum(torch.log(1e-8 + pos_score / neg_score)) / scale
        return con_loss
