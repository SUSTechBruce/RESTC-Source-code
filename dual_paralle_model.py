...
class DualParallModel(torch.nn.Module):
    def __init__(self, args, num_nodes, adj_all, weight_all):
        super(DualParallModel, self).__init__()
    ...
    def compute_scores(self, hidden, mask, seq_emb):
        """
        transformer_emb: 100, dim
        """
        hidden = hidden + seq_emb # transformer的时序表征+GAT的空间关系表征
        ...
        #  seq_emb = seq_emb.unsqueeze(-2).repeat(1, len, 1)
    ...
    def forward(self, inputs, adj, mask_item, item, transformer_seqs, transformer_pos):
        ...
        seq_emb = self.transformer_emb(transformer_seqs, transformer_pos)  # batch, seq, dim
        seq_emb = torch.flip(seq_emb, dims=[1]) # 对seq进行反转
        output = h_local
        # print('seq_len: ', seqs_len) 69
        return output, seq_emb # batch reverse_seq, dim
...
def forward(DualTransGraphModel, data):
    ...
    hidden, seq_emb = DualTransGraphModel(items, adj, mask, inputs, transformer_seqs, transformer_pos) # forward function
    ...
    scores = DualTransGraphModel.compute_scores(seq_hidden, mask, seq_emb)
...
class SparseTransformer(nn.Module):
    ...
    def __init__(self, item_dim, pos_dim, n_items, n_pos, w, atten_way='dot', decoder_way='bilinear', dropout=0,
        ...
        self.atten_transform = nn.Parameter(torch.Tensor(self.dim, self.dim))
        ...
        self.concat_lin = nn.Linear(self.dim * 2, self.dim)
    ...
    def get_alpha(self, session, flag):
        ...
        if flag == 0:
            ...
            alpha_ent = alpha_ent.expand(-1, 70, -1)  # 单一维度扩展 70
            return alpha_ent
        if flag == 1:
    ...
    def global_attention(self, Q_hat, K_hat, V_hat, seq_mask=None, alpha_ent=1):
        ...
        W_trans = self.atten_transform

        A = torch.relu(Q + K + bias)  # batch, seq, dim
        A = torch.matmul(A, W_trans) # batch, seq, dim
        if seq_mask is not None:
    ...
    # def global_seq_attention(self, Q_hat, K_hat, V_hat, seq_mask=None, alpha_ent=1):
    ...
    #         A = A.masked_fill(seq_mask == 0, -np.inf)
    #     A = entmax_bisect(A, alpha_ent, dim=1) 
    #     A = A.expand(-1, -1, K.shape[1]) # batch seq 1 -> batch seq seq 
    #     seq_vec = torch.matmul(A.transpose(1, 2), V_hat) # (batch, 1, seq) * (batch, seq, dim) # 直接concate 改 batch seq seq
    ...
    #     # W_0 = self.atten_w0
    #     # how to do ?
    #     Q = Q.expand(-1, Q.shape[1], -1)
    #     A = torch.matmul(Q, K.transpose(1, 2)) / math.sqrt(self.dim)
    #     # A = torch.relu(Q + K + bias)  # batch, seq, dim
    #     # A = torch.matmul(A, W_0.t()) # batch, seq, 1
    #     # if seq_mask is not None:
    #     #     seq_mask = seq_mask.unsqueeze(-1) # batch, seq, 1
    ...
    #     A = torch.matmul(A, V_hat) # (batch, 1, seq) * (batch, seq, dim) # 直接concate 改 batch seq seq
    #     X = self.self_atten_w1(A) # FFN part of transformer
    #     X = self.activate(X) # nn.relu()
    #     X = self.self_atten_w2(X) # nn.Linear()
    #     X = self.dropout(X)
    ...
    def global_seq_attention(self, Q_hat, K_hat, V_hat, seq_mask=None, alpha_ent=1):
        ...
        # A = A.expand(-1, -1, K.shape[1]) # batch seq 1 -> batch seq seq 
        # seq_vec = torch.matmul(A.transpose(1, 2), V_hat) # (batch, 1, seq) * (batch, seq, dim) # 直接concate 改 batch seq seq
        concat_vec = torch.cat((A, V_hat), dim=-1)

        seq_vec = self.concat_lin(concat_vec)
        # print('vec shape', seq_vec.shape)
    ...
    def get_seq_embed(self, session_vec, last_item):
        seq_len = session_vec.shape[1]
        last_item = last_item.expand(-1, seq_len, -1)

        if self.is_dropout: # self.w_f : nn.Linear (2 * dim, dim)
            transformer_emb = self.dropout(torch.selu(self.w_f(torch.cat((session_vec, last_item), -1)))) 
        else:
            transformer_emb = torch.selu(self.w_f(torch.cat((session_vec, last_item), 2))) # batch seq dim
        # print('seq emb', transformer_emb.shape) ([100, 69, 100])
        seq_emb = transformer_emb/torch.norm(transformer_emb, dim=-1).unsqueeze(2)
        return seq_emb


    def get_transformer_embed(self, session_vec, last_item):
        if self.is_dropout: # batch 1 dim
    ...
    def forward(self, x, pos):
        ...
        global_c = self.global_seq_attention(m_s, x_n, x_s, mask, alpha_global)  # B, 1, dim
        h_t = global_c
        seq_emb = self.get_seq_embed(h_t, m_s) #  # 512, 100
        return seq_emb
