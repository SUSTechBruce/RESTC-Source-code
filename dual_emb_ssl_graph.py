...
class Graph_embedding(nn.Module):
    ...
    def forward(self, global_graph, nodes_feature):
        ...
        # print('original degree shape', global_graph.sum(axis=0).shape)

        # # print('degree shape', degree)
        # r_degree = np.diag(np.power(degree, -0.5))
        # print('re_degree', r_degree)
        # print('re_degree shape', r_degree.shape)
        # # # print('global graph shape', global_graph.shape)
        # # print('r_degree shape', r_degree.shape)
        # # print('global graph shape', global_graph.shape)
        # graph_matrix = r_degree.dot(global_graph).dot(r_degree)
...
class DualSSlGloablModel(torch.nn.Module):
    ...
    def __init__(self, args, global_graph, num_nodes, adj_all, weight_all):
        ...
        self.beta = 0.005 # beta: 0.01 ssl task maginitude  
        ...
        self.w_2_graph = nn.Parameter(torch.Tensor(self.dim, 1))
    ...
    def compute_loss_func(self, targets, reversed_sess_item, gat_hidden, mask, transformer_emb, trans_item_embedding, is_train=True):
        ...
        labels = targets - 1
############################################## 对比学习部分代码  使用Graph = GAT + k-HOP ##########################################################
 
       # using graph_emb = GAT + k-hop
        graph_test_hidden = gat_hidden + k_hop_hidden
        graph_hs = torch.sum(graph_test_hidden * mask, -2) # 第二个维度相加[batch, sequence, hidden]
        graph_hs = graph_hs / torch.sum(mask, 1)  # batch, 1, seq 100, 1, 69
        graph_hs = graph_hs.unsqueeze(-2).repeat(1, len, 1) # s' # 插入transformer 时序表征

        graph_nh = torch.sigmoid(self.glu1_graph(nh) + self.glu2_graph(graph_hs)) # (13)
        graph_beta = torch.matmul(graph_nh, self.w_2_graph)
        graph_beta = graph_beta * mask

        graph_emb = torch.sum(graph_beta * graph_test_hidden, 1)
        # normalization
        # graph_emb = (graph_emb/torch.norm(graph_emb, dim=-1).unsqueeze(1))

        if is_train:
            contrast_loss = self.dual_embs_SSL(graph_emb, trans_emb)
    ...
    def forward(self, inputs, adj, mask_item, item, transformer_seqs, transformer_pos):
        ...
        gat_emb = F.dropout(gat_emb, self.dropout_local, training=self.training)
...
def forward(DualMHTransGraphModel, data, is_train=True):
    ...
    gat_hidden, transformer_emb, trans_item_embeddings = DualMHTransGraphModel(items, adj, mask, inputs, transformer_seqs, transformer_pos) # forward function
...
def train_and_test(epoch, model, train_data, test_data):
    ...
    for data in tqdm(train_loader): 
        ...
        if count % 500 == 0:
            print('The train model loss is: {} The contrastive loss is: {}'.format(dual_fustion_loss, contrast_loss))
    ...
    # if epoch < 14:
    #     model.scheduler_stepLR.step()
    # elif 14 <= epoch and epoch < 20:
    #     print('scheduler_stepLR_1.step()')  
    #     model.scheduler_stepLR_1.step()
    # else:
    #     print('scheduler_crosineAnneal.step()')
    #     model.scheduler_crosineAnneal.step()
    #     # model.scheduler_stepLR_1.step()
